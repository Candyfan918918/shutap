...
BEGIN
  ...
  LOOP
    ...
    v_summary := CASE v_kind
      WHEN 'red_flag' THEN 'Final Verdict: RED FLAG. ' || v_pct || '% of the jury called it.'
      ...
      WHEN 'lawyer_up' THEN 'Final Verdict: Lawyer Up. ' || v_pct || '% said it is that serious.'
      WHEN 'therapy_might_help' THEN 'Final Verdict: Therapy Might Help.'
      WHEN 'need_update' THEN 'Final Verdict: Need Update.'
      ELSE 'The jury was split. No clear verdict.'
    END;
    ...
    -- Lock every verified perspective for this post.
    UPDATE public.post_perspectives
       SET locked_at = now()
     WHERE post_id = r.post_id
    ...
    -- Notify every verified named-party responder.
    INSERT INTO public.notifications (user_id, kind, payload)
    SELECT pp.responder_id,
           'court_verdict',
           jsonb_build_object(
             'case_id', r.id,
             'post_id', r.post_id,
             'verdict', v_kind,
             'percent', v_pct,
             'message', 'Final verdict is in for a case you were named in.'
           )
    FROM public.post_perspectives pp
    WHERE pp.post_id = r.post_id
      AND pp.standing_status = 'verified'
      AND pp.role = 'named_party'
      AND pp.responder_id <> v_author;
