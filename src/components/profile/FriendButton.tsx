...
export function FriendButton({ targetUserId, state, onChanged }: {
  ...
  if (state === "accepted") {
    return (
      <button
        ...
        disabled={busy}
        className="px-3 py-2 rounded-full text-sm font-semibold bg-surface-elevated border border-border disabled:opacity-50"
      >
        🫂 Friends
      </button>
    );
  ...
  if (state === "pending_out") {
    return (
      <button
        disabled
        className="px-3 py-2 rounded-full text-sm font-semibold bg-surface-elevated border border-border opacity-60"
      >
        ⏳ Pending
      </button>
    );
  ...
  if (state === "pending_in") {
    return (
      <button
        ...
        disabled={busy}
        className="px-3 py-2 rounded-full text-sm font-semibold bg-gradient-to-r from-emerald-500 to-emerald-600 text-white disabled:opacity-50"
      >
        ✓ Accept
      </button>
    );
  ...
  return (
    <button
      ...
      disabled={busy}
      className="px-3 py-2 rounded-full text-sm font-semibold bg-surface-elevated border border-border hover:border-primary/60 disabled:opacity-50"
    >
      ➕ Friend
    </button>
  );
